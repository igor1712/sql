...
select mark,count(COLOR)   from AUTO
where MARK = "BMW"
union all
select mark,count(COLOR)  from AUTO
where MARK = "LADA";
...
(select count(mark) from auto au where au.mark = a.mark ) as "Всего",
(select count(mark) from auto au where au.mark != a.mark ) as "Остальные"
 from AUTO a;
